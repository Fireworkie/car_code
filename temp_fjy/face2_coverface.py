...
face_img = cv2.imread("C:\\Users\\19403\\Desktop\\picture\\woman.jpg")  # 读取人脸图像
glass_img = cv2.imread("C:\\Users\\19403\\Desktop\\picture\\glasses2.jpg", cv2.IMREAD_UNCHANGED)  # 读取眼镜图像，保留图像类型
height, width, channel = glass_img.shape  # 获取眼镜图像高、宽、通道数
# 加载级联分类器
face_cascade = cv2.CascadeClassifier("./cascades/haarcascade_frontalface_default.xml")
grayframe = cv2.cvtColor(face_img, cv2.COLOR_BGR2GRAY)  # 转为黑白图像
faces = face_cascade.detectMultiScale(grayframe, 1.3, 5)  # 识别人脸
for (x, y, w, h) in faces:  # 遍历所有人脸的区域
    gw = w  # 眼镜缩放之后的宽度
    gh = int(height * w / width)  # 眼镜缩放之后的高度
    glass_img_resized = cv2.resize(glass_img, (gw, gh))  # 按照人脸大小缩放眼镜
    face_img = overlay_img(face_img, glass_img_resized, x, y + int(h * 1 / 3))  # 将眼镜绘制到人脸上
