...
# 定义舵机
pan = AngularServo(12, min_angle=-90, max_angle=90, min_pulse_width=0.5/1000,max_pulse_width=2.5/1000)
tilt = AngularServo(15, min_angle=-90, max_angle=90, min_pulse_width=0.5/1000,max_pulse_width=2.5/1000)
...
def obj_center(cascade_path,objX, objY, centerX, centerY):
	...
	while True:
		...
		if rect is not None:
			...
			# 显示图像
			cv2.namedWindow("Pan-Tilt Face Tracking (Blurred)", cv2.WINDOW_NORMAL)
	# 设置窗口的宽度和高度，这里以640x480为例
			window_width = 1280
			window_height = 960
			cv2.resizeWindow("Pan-Tilt Face Tracking (Blurred)", window_width, window_height)
